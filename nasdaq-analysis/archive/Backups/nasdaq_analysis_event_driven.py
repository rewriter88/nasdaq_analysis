# ===========================================================
# Event-Driven Nasdaq Analysis - Rebalance Only on Composition Changes
# • Uses Yahoo Finance monthly data for tracking changes
# • Rebalances only when top N composition changes (not monthly)
# • Allows weight drift between rebalancing events for momentum
...
# ------------------------------------------------------------------
# 3) Event-driven weight matrix builder (rebalance only on changes)
# ------------------------------------------------------------------
def build_event_driven_weights(schedule, universe, index):
    """Build weight matrix with event-driven rebalancing (only on composition changes)."""
    ...
    for i, (start, end, names) in enumerate(schedule):
        ...
        w.loc[rebalance_date, names] = 1.0 / len(names)
        
        print(f"  Rebalance on {rebalance_date.date()}: {names} (equal {1.0/len(names):.3f} each)")
...
# Create event-driven weights for each strategy
print("Top 1 rebalancing events:")
weights_top1 = build_event_driven_weights(top1_schedule, prices.columns, idx)

print("Top 2 rebalancing events:")
weights_top2 = build_event_driven_weights(top2_schedule, prices.columns, idx)

print("Top 3 rebalancing events:")
weights_top3 = build_event_driven_weights(top3_schedule, prices.columns, idx)

print("Top 4 rebalancing events:")
weights_top4 = build_event_driven_weights(top4_schedule, prices.columns, idx)

print("Top 5 rebalancing events:")
weights_top5 = build_event_driven_weights(top5_schedule, prices.columns, idx)

print("Top 6 rebalancing events:")
weights_top6 = build_event_driven_weights(top6_schedule, prices.columns, idx)

print("Top 7 rebalancing events:")
weights_top7 = build_event_driven_weights(top7_schedule, prices.columns, idx)

print("Top 8 rebalancing events:")
weights_top8 = build_event_driven_weights(top8_schedule, prices.columns, idx)

print("Top 9 rebalancing events:")
weights_top9 = build_event_driven_weights(top9_schedule, prices.columns, idx)

print("Top 10 rebalancing events:")
weights_top10 = build_event_driven_weights(top10_schedule, prices.columns, idx)

# ------------------------------------------------------------------
# 4) Event-driven portfolio calculation with weight drift
# ------------------------------------------------------------------
def portfolio_cum_event_driven(weights):
...
print("\nCalculating event-driven portfolio performance...")

# Calculate performance curves with weight drift
curve_top1 = portfolio_cum_event_driven(weights_top1)
curve_top2 = portfolio_cum_event_driven(weights_top2)
curve_top3 = portfolio_cum_event_driven(weights_top3)
curve_top4 = portfolio_cum_event_driven(weights_top4)
curve_top5 = portfolio_cum_event_driven(weights_top5)
curve_top6 = portfolio_cum_event_driven(weights_top6)
curve_top7 = portfolio_cum_event_driven(weights_top7)
curve_top8 = portfolio_cum_event_driven(weights_top8)
curve_top9 = portfolio_cum_event_driven(weights_top9)
curve_top10 = portfolio_cum_event_driven(weights_top10)

# QQQ benchmark (buy and hold)
rets = prices.pct_change().fillna(0)
...
plt.xlabel("Date")
plt.title("Nasdaq Top 1-10 Event-Driven Strategies • Jul 2005 – Jun 2025\nRebalance Only on Composition Changes • Final Portfolio Values (Starting $100K)")
...
# 6) Summary table
# ------------------------------------------------------------------
print(f"\nPerformance Summary (Event-Driven Rebalancing)")
print("=" * 60)
...
print(f"• Rebalancing: Event-driven (only when composition changes)")
...
print(f"• Total rebalancing events: {sum(len(schedule) for schedule in all_schedules)}")

# ------------------------------------------------------------------
# 7) Rebalancing event summary
# ------------------------------------------------------------------
print(f"\nRebalancing Event Summary:")
print("=" * 50)
...
all_schedule_names = [
    ...
    ("Top 8", top8_schedule),
    ("Top 9", top9_schedule),
    ("Top 10", top10_schedule),
]

for strategy_name, schedule in all_schedule_names:
    print(f"\n{strategy_name}: {len(schedule)} rebalancing events")
